...
5. 识图：`POST /v1/vision/look`（multipart `image` + 可选 `question`）；图片仅进程内 TTL 缓存，不落库。
6. 同图追问：`POST /v1/vision/follow-up`（`conversation_id` + `text`）→ `{ reply_text }`，仍用 `COCO_VISION_MODEL`。
7. ASR / TTS（追问外壳）：`POST /v1/audio/transcriptions`、`POST /v1/audio/speech`；模型可配 `COCO_ASR_MODEL`、`COCO_TTS_MODEL`、`COCO_TTS_VOICE`。
