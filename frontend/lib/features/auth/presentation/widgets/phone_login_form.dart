...
/// 登录表单交互：发码 / 登录 / 返回选身份。
class PhoneLoginFormHandlers {
  const PhoneLoginFormHandlers({
    required this.requestingCode,
    required this.loggingIn,
    required this.onRequestCode,
    required this.onLogin,
    required this.onBackToRole,
    this.showBackToRole = true,
  });
  ...
  final bool showBackToRole;

  bool get formBusy => requestingCode || loggingIn;
}

/// 家长端：客厅模糊底 + 白卡内含主次按钮。
class ParentLoginScaffold extends StatelessWidget {
  const ParentLoginScaffold({
    super.key,
  ...
  final PhoneLoginFormHandlers handlers;
...
class ChildLoginScaffold extends StatelessWidget {
  const ChildLoginScaffold({
    super.key,
    required this.state,
    required this.phoneController,
    required this.codeController,
