...
- **每次创建 git commit 时**，将 `frontend/lib/core/app/app_version_config.dart` 的 `AppVersionConfig.label` **小版本 +1**（第三位：`v1.0.0` → `v1.0.1`）；该 commit 须包含此版本变更。仅文档/注释且无发版意义的 commit 可不改。
...
- 子女端：结论优先、现代克制、青绿主色、底部三栏（近况 / 留言 / 家庭）。
...
- 创建提醒、分享给家人等动作，必须经用户明确确认后由业务 API 执行。用户主动要求记住时，语音工具 `save_memory` 直接写入 `coco.memories`（不走确认大卡）；Mem0 仍在挂断后自动整理，仅用于开场注入与检索，不出现在记忆页。父母可在记忆页查看/删除显式记忆。
...
1. 格式化变更代码。
2. 若本次要提交 git：**小版本号已 +1**（见 `app_version_config.dart`），并与功能变更在同一 commit。
3. 后端相关：迁移可 upgrade；关键接口可 curl 通。
4. 前端相关：`flutter analyze` 通过；iOS 模拟器能跑通当前流程。
5. 说明已完成内容、未验证风险、后续必要工作。
